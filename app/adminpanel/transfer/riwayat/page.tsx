...
export default function RiwayatTransferPage() {
  ...
  const handleConfirm = async () => {
    ...
    setUpdating(true)
    try {
      const res = await fetch(`/api/admin/transfers/${confirmAction.transfer.id}`, {
        method: "PATCH",
      ...
      if (confirmAction.newStatus === "selesai") {
        fetch("/api/admin/initial-data")
          ...
          .catch(() => { })
      } else if (confirmAction.newStatus === "dibatalkan") {
        fetch("/api/admin/initial-data")
          .then((r) => r.json())
          .then((refreshData) => {
  ...
  // Pengirim boleh membatalkan jika masih menunggu dan outlet yang dipilih adalah outlet asal
  const canCancel = (transfer: Transfer) => {
    if (isSuperAdmin) return false
    if (!selectedOutletId || selectedOutletId === "all") return false
    return transfer.status === "pending" && transfer.fromOutletId === selectedOutletId
  }
                          ...
                          <div className="flex items-center justify-center gap-1">
                            ...
                            {canCancel(transfer) && (
                              <Button
                                variant="ghost"
                                size="sm"
                                onClick={() => openConfirm(transfer, "dibatalkan")}
        ...
        <DialogContent className="max-w-lg">
          ...
          {selectedTransfer && (
            <div className="flex flex-col gap-4">
              ...
              {selectedTransfer.status === "pending" && canCancel(selectedTransfer) && (
                <div className="flex justify-end border-t pt-3">
                  <Button
                    size="sm"
                    variant="destructive"
                    onClick={() => {
                      setDetailOpen(false)
                      openConfirm(selectedTransfer, "dibatalkan")
                    }}
                  >
          ...
          <DialogHeader>
            <DialogTitle className="font-heading">
              {confirmAction?.newStatus === "diterima"
                ...
                : confirmAction?.newStatus === "dibatalkan"
                  ? "Konfirmasi Batalkan Transfer"
            ...
            <DialogDescription>
              {confirmAction?.newStatus === "diterima"
                ...
                : confirmAction?.newStatus === "dibatalkan"
                  ? `Apakah Anda yakin ingin membatalkan transfer ${confirmAction?.transfer.id}? Status akan berubah menjadi "Dibatalkan".`
