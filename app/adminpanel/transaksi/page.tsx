...
export default function TransaksiBaruPage() {
  ...
  const { selectedOutletId, outlets, products, nextInvoiceNumber, currentUser, isSuperAdmin, reloadInitialData } = useOutlet()
  ...
  // Step 2 - Customer (invoice fetched from DB)
  const [invoice, setInvoice] = useState(() => nextInvoiceNumber())
  ...
  // Fetch invoice number from DB on mount
  useEffect(() => {
    let cancelled = false
    fetch("/api/admin/transaksi?action=next-invoice")
      .then((r) => r.json())
      .then((data) => {
        if (!cancelled && data.invoice) setInvoice(data.invoice)
      })
      .catch(() => { })
  ...
  const processTransaction = async () => {
    ...
    try {
      ...
      // Reload data from DB to get fresh stock & transaction list
      await reloadInitialData()
      router.push("/adminpanel/transaksi/riwayat")
    } catch {
  ...
  // Super Admin cannot create transactions
  if (isSuperAdmin) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 py-20">
        ...
        <p className="text-center text-muted-foreground">
          Super Admin tidak dapat membuat transaksi.<br />
          Silakan login sebagai Admin Outlet untuk membuat transaksi.
        </p>
        <Button variant="outline" onClick={() => router.push("/adminpanel")}>
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center justify-between">
        ...
        {step < 2 ? (
          ...
          <Button onClick={processTransaction} disabled={!canProcess || isSubmitting} className="gap-2">
            {isSubmitting ? (
              <>
